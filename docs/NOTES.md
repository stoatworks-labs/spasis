...
6. **No `sptest --shaders` in CI.** A GitHub macOS runner cannot create an
   ...
   the class of bug `gl_PointCoord` was, and no CI machine here has one — that
   check exists locally and nowhere else.

**Closed since 2026-08-26:** CI and the release workflow (`.github/workflows/`),
the website entry in `projects.json`, and `tools/verify.sh`.
`source/StoatworksAbout.h` is now generated by `sync-about.py` rather than
hand-written.
